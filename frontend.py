...
def create_main_menu_markup():
    ...
    markup.row(types.InlineKeyboardButton(text='Посмотреть расписание',
                                          callback_data=create_callback_data(States.S_SHOW_SCHEDULE_CHOOSE_DAY)))
    markup.row(types.InlineKeyboardButton(text='Забронировать',
                                          callback_data=create_callback_data(States.S_RESERVE_CHOOSE_ROOM)))
    markup.row(types.InlineKeyboardButton(text='Отменить бронирование',
                                          callback_data=create_callback_data(States.S_CANCEL_CHOOSE_RESERVE)))
    return markup
...
def create_choose_room_markup():
    ...
    for room in rooms:
        callback_data = create_callback_data(States.S_RESERVE_CHOOSE_DAY, r=room[0])
        markup.add(types.InlineKeyboardButton(text=room[1],
                                              callback_data=callback_data))
    markup.add(types.InlineKeyboardButton(text='Назад', callback_data=create_callback_data(States.S_MAIN_MENU_REPLACE)))
    return markup
...
def create_choose_day_markup(tm, next_state, room_id=None):
    ...
    row = [types.InlineKeyboardButton(text='Назад', callback_data=create_callback_data(States.S_RESERVE_CHOOSE_DAY))]
    if next_state == States.S_RESERVE_CHOOSE_START_TIME:
        row = [
            types.InlineKeyboardButton(text='Назад', callback_data=create_callback_data(States.S_RESERVE_CHOOSE_ROOM)),
            types.InlineKeyboardButton(text='Выбрать дату',
                                       callback_data=create_callback_data(States.S_RESERVE_CALENDAR, r=room_id))]
    else:
        row = [types.InlineKeyboardButton(text='Назад', callback_data=create_callback_data(States.S_MAIN_MENU_REPLACE)),
               types.InlineKeyboardButton(text='Выбрать дату',
                                          callback_data=create_callback_data(States.S_SHOW_SCHEDULE_CALENDAR))]
...
def create_calendar_markup(tm, next_state, room_id=None):
    ...
    month_button = types.InlineKeyboardButton(text=f'{month_list[month - 1]} {year}',
                                              callback_data=create_callback_data(States.S_DO_NOTHING))
    ...
    for weekday in weekdays:
        week_row.append(
            types.InlineKeyboardButton(text=weekday, callback_data=create_callback_data(States.S_DO_NOTHING)))
    markup.row(*week_row)
    ...
    for week in month_calendar:
        ...
        markup.row(*week_row)
    if next_state == States.S_RESERVE_CHOOSE_START_TIME:
        prev_state = States.S_RESERVE_CHOOSE_DAY
    else:
        prev_state = States.S_SHOW_SCHEDULE_CHOOSE_DAY
    navigate_row = [types.InlineKeyboardButton(text='<--',
                                               callback_data=create_callback_data(States.S_CHANGE_MONTH,
                                                                                  tm=prev_month_unix,
                                                                                  ns=next_state,
                                                                                  r=room_id)),
...
def create_hour_choose_menu(next_state, tm, room_id):
    ...
    hourslist = []
    if next_state == States.S_RESERVE_CHECK:
        prev_state = States.S_RESERVE_CHOOSE_START_TIME
...
def create_cancel_markup(user_id):
    ...
    for reserve in reserves:
        ...
        callback_data = create_callback_data(States.S_CANCEL_RESERVE, tm=reserve[0], u=user_id, r=room_id)
        markup.add(types.InlineKeyboardButton(text=button_text, callback_data=callback_data))

    markup.add(
        types.InlineKeyboardButton(text='Главное меню', callback_data=create_callback_data(States.S_MAIN_MENU_REPLACE)))
...
def create_button_main_menu_comment_markup(room_id, tm_start):
    ...
    markup.row(types.InlineKeyboardButton(text='Добавить комментарий',
                                          callback_data=create_callback_data(States.S_RESERVE_ADD_COMMENT,
                                                                             r=room_id,
                                                                             tms=tm_start)))
